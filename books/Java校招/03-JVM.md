...
- 在采用经典分代布局的收集器中，通常按**新生代**（Eden + 两个 Survivor）与**老年代**理解；Eden:Survivor 的 8:1:1 是 Serial/Parallel 等实现的常见示例，不是所有收集器或所有版本的固定比例。G1 以 Region 组织堆，ZGC 的实现细节也不同；
...
- 直接内存严格说不属于 JVM 运行时数据区，但常与 NIO 一起考察：`ByteBuffer.allocateDirect`、Netty 的堆外内存；通常受 `-XX:MaxDirectMemorySize` 约束，默认值和实际分配还取决于 JDK/VM 实现，不能简单当作永远等于 `-Xmx`。
...
- 应用：经典分代收集器的新生代。常见示意是 Eden:S0:S1 = 8:1:1，每次把 Eden + 一块 Survivor 的存活对象复制到另一块 Survivor；具体比例、年龄晋升和是否存在独立 Survivor 区由收集器实现决定。
