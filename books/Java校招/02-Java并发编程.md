...
这张图是 HotSpot 早期实现的面试化示意，不是 Java 语言规范保证的状态机。偏向锁自 JDK 15 起默认禁用并在后续版本移除相关实现；现代 JDK 不应假定每次竞争都会经过“偏向→轻量级→重量级”。“只升级不降级”也只是经典实现的简化记忆，实际锁膨胀、撤销和消除由具体 HotSpot 版本与 JIT 决定。
...
- 问：锁升级过程？
- 答：面试可以先讲经典 HotSpot 示意：无竞争时走轻量路径，竞争加剧时可能膨胀为 monitor；偏向锁是旧版本优化，JDK 15 起默认禁用，现代 JDK 不应把“偏向→轻量→重量”当成必经路径。真正能否消除、膨胀、阻塞以及何时自旋，取决于 JDK/HotSpot 实现和运行时竞争，语言层只保证 synchronized 的互斥、可见性与可重入语义。
- 问：synchronized 可重入吗？
