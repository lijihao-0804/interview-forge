...
| REPEATABLE READ（可重复读） | 解决 | 解决 | 视读取方式而定（快照读依靠固定 ReadView；当前读在合适的索引和范围条件下使用记录锁/间隙锁/临键锁） |
| SERIALIZABLE（串行化） | 解决 | 解决 | 解决 |

MySQL InnoDB 默认 **REPEATABLE READ**：

- 快照读（普通 SELECT）靠 **MVCC** 复用 ReadView，得到一致的快照；它不会把后来提交的新行加入当前快照；
- 当前读（`SELECT ... FOR UPDATE`、`UPDATE`、`DELETE`）会加锁；RR 下范围条件可能使用**间隙锁/临键锁**防止并发插入，但具体锁型取决于索引、谓词和访问路径；
...
- 问：MySQL 默认隔离级别？
- 答：InnoDB 默认是 REPEATABLE READ；快照读用 MVCC 保持同一 ReadView，当前读则依赖访问路径上的记录锁/间隙锁/临键锁，不能脱离索引和谓词笼统地说“必然加间隙锁”。
- 问：RC 和 RR 怎么选？
...
- 问：行锁锁的是什么？
- 答：InnoDB 的记录锁作用在索引记录（聚簇索引或二级索引项）上；没有合适索引时，扫描和加锁范围可能很大，甚至覆盖大量记录及间隙，表现为严重阻塞，但这不等于真正获得了一把表锁。InnoDB 仍然是在索引层面加锁。
- 问：间隙锁是做什么的？
...
**key_len 算例**：联合索引 `(a int, b varchar(20) utf8mb4)` 中，`key_len` 会受数据类型、字符集、是否可空以及变长字段长度字节影响；实际应以 `EXPLAIN` 输出为准，不要只按“用了几列”猜数值。`EXPLAIN ANALYZE`（MySQL 8.0.18+）还能看实际执行耗时与行数，比普通 `EXPLAIN` 的估算更接近真实执行。
...
| 隔离 | RR 怎么减少幻读 | 快照读复用 ReadView；当前读按索引和谓词使用记录/间隙/临键锁 |
