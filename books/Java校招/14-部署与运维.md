...
> 示例以 Ubuntu 22.04 / Docker 24 / Kubernetes 1.29 / Jenkins 2.x（LTS 2.4xx）为基线；Kubernetes API、镜像和插件版本变化较快，部署前以目标集群文档为准。
...
**调度与 HPA**：kube-scheduler 按节点资源、亲和性、污点容忍选节点；`nodeSelector`/节点亲和性/污点容忍控制落点。HPA（水平扩缩容）需要可用的 Metrics API（常见实现是 metrics-server）和合适的资源 `requests`；现代 `autoscaling/v2` 示例使用 `metrics.resource.target.averageUtilization`，`targetCPUUtilizationPercentage` 是旧版字段，不应在新清单中照抄。目标值也不是所有集群固定 80%，应按 SLO、容量和压测设定。requests/limits 决定 QoS 三类：Guaranteed（requests=limits）/ Burstable（requests<limits）/ BestEffort（都不设）。
