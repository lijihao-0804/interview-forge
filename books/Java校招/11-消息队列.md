...
> 示例以 Kafka 3.x / RocketMQ 5.x / RabbitMQ 3.12 为基线；吞吐、默认值和具体 API 会随版本、消息大小、持久化与部署拓扑变化，回答时应说明口径。
...
| 至少一次 at-least-once | 在设计的故障范围内尽量不丢，但可能重复 | 手动 ACK/提交 + 失败重投（主流选择） |
| 精确一次 exactly-once | 只在明确的事务/处理边界内声称一次效果 | 事务/幂等能力 + 处理端与外部系统配合，代价最高 |
...
| 吞吐 | 高（常用于大吞吐日志流；实际取决于分区、消息大小、磁盘和副本） | 高（实际取决于队列、刷盘和复制配置） | 中到高（实际取决于交换机、队列、确认和持久化配置） |
| 延迟 | 通常为毫秒级，需结合批量与刷盘配置 | 通常为毫秒级，需结合发送和存储配置 | 通常为毫秒到更低，需以实测和持久化要求为准 |
...
- 发送成功要有 **confirm 回调**：Broker 按配置完成持久化/复制后返回确认，失败或超时重发；
- 配置重试次数与重试间隔，重试仍失败进本地重试表，定时补偿；
- Kafka 用 `acks=all`（并结合合理的 `min.insync.replicas`）+ `retries>0`；RocketMQ 用同步发送 + sendResult 校验；RabbitMQ 用 publisher confirm，并正确配置 durable queue 与 persistent message。

### 4.2 Broker 端

- **持久化**：消息落盘（Kafka 落分区日志、RocketMQ 落 CommitLog、RabbitMQ 落队列文件）；
- **多副本**：主副本按配置等待从副本或仲裁副本后再确认（Kafka ISR、RocketMQ 主从同步、RabbitMQ 镜像/仲裁队列）；
...
消息记录与业务数据在同一个数据库事务里，“业务成功则待投递消息记录一定存在”；投递异步化、失败可补偿。它保证的是应用数据库中的消息记录与业务事务一致，不等于消息已经到达 Broker，也不等于可以无限重试。
