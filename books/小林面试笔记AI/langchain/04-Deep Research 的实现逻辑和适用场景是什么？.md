...
> 原文：[Deep Research 的实现逻辑和适用场景是什么？](https://xiaolinnote.com/ai/langchain/deep_research.html) · 小林面试笔记
...
并行的是相互独立的搜证据任务，不是无条件并行写最终章节；循环还必须受预算、停止条件和人工复核约束。
