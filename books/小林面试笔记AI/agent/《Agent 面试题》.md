...
> **校订说明（学习版）**：本文把 Agent 作为“模型 + 工具 + 状态/记忆 + 运行时控制”的工程系统来讲，而不是把它等同于某个模型或某个框架。不同框架对 Agent、Workflow、Memory、Reflection 的命名和边界并不完全一致；面试时应说明自己的定义、停止条件、权限边界和评估方式。

```mermaid
flowchart LR
    U[用户目标] --> S[State / Context\n目标、预算、历史、权限]
    S --> M[模型决策\n回答 / 工具调用 / 结束]
    M --> V{运行时校验\nSchema、权限、预算、幂等}
    V -->|允许| T[Tool / 外部系统执行]
    V -->|拒绝或需确认| H[人工确认 / 安全拒绝]
    T --> R[结果或错误]
    R --> S
    S --> C{停止条件\n完成、失败、超时、次数上限}
    C -->|继续| M
...
图中最容易被忽略的是“运行时校验”和“停止条件”：模型提出动作，程序决定是否允许并执行；工具结果再回到状态，直到达到业务验收条件或触发失败/预算边界。
...
你直接调用模型的 chat 接口，默认通常是“一次请求、一次响应”：你给它一个输入，它生成一个输出，然后由调用方决定是否继续。多轮对话也不等于模型天然拥有持久状态；若没有外部上下文、工具和运行时，模型不会主动发起后续动作，也不会自动知道上一轮发生了什么。它可以提出“先查资料再汇报”的方案，但实际查找和后续调度仍需由程序或宿主应用完成。
...
这三个边界需要分开看：知识可能过时，模型本身没有外部执行权限，也不会自动持久化任务状态。单次模型调用仍能完成不少复杂推理，但要可靠地完成多步骤、可验证、会改变外部世界的任务，必须在调用外增加工具、状态管理、运行时控制和验收逻辑。
...
**第二件：记忆与状态管理**。一次模型调用只看调用方传入的上下文；多轮对话能否“记住”取决于应用是否保存并重新注入历史。Agent 系统通常区分当前任务的短期状态（目标、工具结果、待办、错误、预算、审批状态）和跨任务的长期存储（偏好、事实、事件、文档或经验）。长期数据可以放在关系库、KV、对象存储、全文/向量索引或事件流中，是否使用向量检索要由查询类型和一致性要求决定。有了明确的状态和恢复策略，Agent 才能在长任务中保持连贯并支持重试、暂停和审计。
...
你可能会问，Agent 的概念其实很早就有了，为什么近几年工程讨论明显增多？更稳妥的解释是：模型的指令遵循和工具调用能力、外部系统的 API 化、以及编排/观测基础设施逐步成熟，降低了把闭环做成产品的门槛；具体的“爆发时间”并不是一个可由单一事件证明的结论。

第一个条件是模型在部分任务上跨过了“能用”的门槛。与早期模型相比，较新的通用模型和推理模型通常更擅长遵循结构化指令、产生工具调用参数和处理多步上下文，但能力仍随模型、任务和工具定义变化，不能据此保证 Agent 一定可靠。

第二个条件是工具调用逐步标准化。许多模型 API 提供了结构化工具调用或函数调用接口，但 schema、并行调用、严格校验和错误行为并不完全统一；运行时仍需做适配、校验、权限和重试处理。

第三个条件是配套生态的完善。编排框架、数据/检索基础设施、可观测性和外部 API 降低了部分开发门槛，但它们不会自动解决状态一致性、数据权限、评估和运营问题。

#### Agent 生态的最新趋势

Agent 火起来之后，一个很自然的问题就冒出来了：Agent 越来越多，它们之间怎么协作？工具越来越多，怎么统一管理？这两个问题催生了两个非常重要的标准协议，面试里被问到的概率很高，值得了解一下。

第一个常被讨论的是 MCP（Model Context Protocol，模型上下文协议）。可以把它理解为 Agent 应用连接工具和资源的一种接口约定，但它不是安全策略或业务编排本身。

在缺少共同接口时，框架与工具提供方往往需要各自编写适配层。MCP 通过协议约定减少一部分重复适配；但兼容 Host/Client 仍要处理版本、认证、权限、参数、超时、错误和审计，不能理解为“任何支持者都无需额外工程就能互通”。

在采用 MCP 的实现中，常见的抽象包括 Host（承载用户交互和权限策略的应用）、Client（维护与 Server 的连接和协议交互）以及 Server（暴露工具、资源或提示模板的一侧）；具体部署可以把多个角色放在同一进程，也可以拆开。

![](../images/74a893b575ac5e0adccbb27f.png)

MCP 的治理组织、兼容实现和公开 Server 数量会随时间变化，不能把某个时间点的组织关系或目录数量当成协议本身的定义。工程上更重要的是核对目标 Host/Client/Server 的协议版本、认证方式、权限模型、超时和审计能力。

另一个常被讨论的是 A2A（Agent-to-Agent）一类的 Agent 间通信协议。如果说 MCP 主要描述 Agent/应用如何接入工具或资源，A2A 类协议主要关注 Agent 之间如何发现能力、传递任务和回传结果；二者的具体版本、治理与互操作性需要按目标实现核对。

在多 Agent 系统里，不同 Agent 可能来自不同厂商或框架，需要约定能力发现、任务输入、进度、结果和错误。在一些 A2A 实现中可以看到类似 Agent Card 的能力描述，但具体字段、状态模型和互操作范围要按目标协议版本核对。

是否由哪些组织维护、哪些厂商接入，属于会变化的生态事实；不要把厂商支持列表当作跨框架互操作已经成立的证明。真正落地时仍要验证身份、授权、任务幂等、状态回调、错误协议和数据边界。

这两个协议可以按边界理解为互补：MCP 偏向“应用/Agent 与工具、资源之间的连接”，A2A 类协议偏向“Agent 与 Agent 之间的任务通信”。它们不是安全边界，也不自动解决授权、数据泄露、重试幂等和结果可信度；面试时把定位与未解决的问题一起说清楚更准确。
...
在教学上可以先用四个核心组件来理解 Agent：LLM、工具、状态/记忆、规划或编排模块；实际系统还常需要运行时、权限、观测、重试和人工审批组件。

LLM 负责根据输入和状态提出决策；工具是受控的外部能力边界；状态/记忆保存当前任务和跨任务数据；规划/编排负责拆解、路由和汇总。真正让系统可运行的，还包括对模型输出做 schema、权限、预算和幂等校验的 runtime。
...
你可以把整个 Agent 系统类比成一家公司：**LLM 是决策顾问**，提出下一步；**工具系统是受权限约束的执行团队**；**状态/存储系统是工作台和档案室**；**规划/编排模块负责拆解、调度和汇总**；**runtime 则像法务、财务和审计**，在动作真正发生前检查权限、预算和风险。四个教学组件加上运行时边界，才撑起可控的自主运行能力。
...
不同模型之间的差异要用任务评测来确认。Agent 需要做多步决策，模型的推理、指令遵循、结构化输出和工具调用稳定性都会影响结果；能力更强或推理优化的模型可能更稳，但调用成本和延迟也可能更高，不能只凭型号判断。

这里还有一类值得关注的模型：针对复杂推理或长程决策优化的模型。它们可能更适合规划和关键判断，但并非所有工具调用、结构化输出或低延迟场景都更优；应按任务成功率、格式合规率、延迟和成本选择。
...
然后是 **工具系统**，这是 Agent 与许多外部系统交互的主要受控边界，但不是唯一的状态或约束来源；运行时还可能接入策略引擎、缓存、审批和事件系统。
...
MCP（Model Context Protocol，模型上下文协议）是一类用于连接应用与外部能力的协议；常见抽象包括 Tools（可执行的操作）、Resources（可读取的数据）和 Prompts（可复用的提示模板）。协议版本、消息细节和支持范围应以目标实现的官方文档为准，不要把某个客户端的能力当成所有实现的共同保证。

在采用 MCP 的实现中，常见的架构抽象是 Host（承载交互、配置和权限策略的应用）、Client（维护协议连接的一侧）和 Server（暴露工具、资源或提示的一侧）；这些角色可以拆进不同进程，也可以组合部署。

工具提供方实现 Server 后，兼容的 Host/Client 才能按协议发现和调用能力；仍可能需要认证、权限映射、参数适配、超时和错误处理。可以把 MCP 理解成接口约定，而不是“接上就自动安全互通”的保证。

MCP 的治理组织、兼容实现和公开 Server 数量会随时间变化。工程落地时更应核对目标实现的协议版本、认证方式、最小权限、资源隔离、超时和审计能力。
...
然后是**长期存储/记忆**，可以使用关系数据库、KV、对象存储、全文索引、向量索引或事件流。只有需要语义召回的内容才适合 embedding 后进入向量索引，并同时保存原文、来源、时间、租户和权限元数据。

这就像人的「长期记忆」，容量大、可以跨天保留，但需要主动「回忆」才能调出来。

这里借用认知科学对人类长期记忆的分类来帮助理解 Agent 数据组织（这是教学类比，不是统一的工程标准；实现可能是结构化查询、全文/向量检索、事件回放或它们的组合）：
...
如果什么都往长期存储里塞，检索出来的噪音会很多，反而干扰模型的决策；如果存得太少，又失去了长期记忆的意义。可以在写入前做重要性、来源、敏感性和重复性评估，并按业务决定是否加入时间衰减（Memory Decay）；衰减只是排序特征，不能覆盖明确的有效期、合规保留或版本规则。
...
最基础的是 **CoT**（Chain of Thought，思维链）这一类逐步推理方法：让模型在生成结论前处理若干中间步骤。教学上可以要求简洁的步骤、计划或可验证的中间结果，但不要把“展示完整隐藏推理”当成生产接口。

为什么逐步处理有时能提升效果？因为中间结果会成为后续生成的上下文，帮助模型分解问题；收益并不稳定，仍需用任务级评测验证，而且额外输出会增加 token、延迟和泄露风险。
...
LangChain、LlamaIndex、AutoGen 等框架都覆盖 Agent 或编排的一部分，但抽象边界、状态模型、运行时和版本演进不同；选型时应按任务、部署、观测、持久化和团队熟悉度比较，而不是假设它们只是同一套四组件的不同包装。
...
第二个雷是对记忆的理解太浅，「记忆就是上下文」这个回答不完整。更准确的说法是：当前任务状态需要以适合运行时的方式保存并按需注入上下文；跨任务数据则根据结构和查询方式放在关系库、KV、对象存储、全文/向量索引或事件系统中，两者机制和用途不同。
...
Agent 的运行方式是一个反复循环的过程：**决策（可理解为 Thought）-> 行动（Action）-> 看结果（Observation）-> 再决策……** 直到模型或验收器判断任务完成，或运行时触发失败/预算/超时边界。Thought 是论文和教学中的概念，不代表生产系统必须暴露完整隐藏推理。
...
常见的停止条件有这几种：第一种是模型或验收器判断目标已达成；第二种是设置最大循环次数，超过后按失败或部分完成收尾；第三种是设置 token、工具调用次数或金额预算；第四种是超时、取消和人工接管。具体阈值应根据任务的 p95 延迟、成本预算和失败代价配置，不能把某个示例数字当作通用标准。实际工程里这些机制通常同时存在，哪个先触发就记录原因并安全收尾。
...
所以在生产环境里，很多团队会给 Agent 加上详细的执行日志，记录状态快照、模型请求/响应元数据、工具调用、校验结果和终止原因；不应默认持久化或暴露模型的完整隐藏推理内容，还要按隐私与合规要求脱敏。
...
因此工程上常见的一种折中是**「Agentic Workflow」**：**用 Workflow 固定主流程的骨架，在需要灵活判断的节点嵌入 Agent，其余固定节点直接用 LLM 或 Tools。** 骨架较确定，便于调试和设预算；局部节点保留灵活性。但它不是所有生产系统的默认答案，仍要按任务不确定性、风险和维护成本选择。
...
它的名字直接说明了它的核心机制：把决策（Reasoning）和行动（Acting）交替进行。具体来说，ReAct 的每一轮循环可以抽象为 Thought/Decision -> Action -> Observation；生产实现通常记录结构化决策、工具调用和结果，不必把完整内部推理原样输出。

Thought/Decision 阶段，模型根据目标、状态和工具 schema 选择下一步，必要时输出简洁的理由或计划字段；Action 阶段，模型提出工具名和参数；Observation 阶段，运行时校验、执行工具并把结果或错误反馈给模型，随后进入下一轮决策。

把决策、动作和观察分开，有助于运行时做 schema、权限和预算校验，也便于按事件追踪“模型提出了什么、程序执行了什么、结果如何”。分开不保证决策正确，也不要求暴露完整思维链；循环会在验收通过、模型请求结束或硬性边界触发时停止。
...
Reflexion 类机制的收益高度依赖任务、反馈信号、基座模型和评测设置；论文或基准中的提升不能直接外推到业务系统。工程上每增加一轮反思都会增加模型调用、工具执行或延迟，因此应通过带验收标准的离线评测比较“质量提升 / 成本 / 延迟”，并设置预算和停止条件。
...
实际工程里，纯 Agent 模式需要更强的权限、预算、观测和验收约束；在探索性或路径高度不确定的任务中仍可能合适。

![](../images/36a08f69d220dbdc4591784c.png)

一种常见的做法是**「Agentic Workflow」**，整体用 Workflow 框住主流程，在需要灵活处理的节点嵌入 Agent 能力。比如一个客服系统，意图识别 -> 知识检索 -> 回答生成这条主链路可以固定，但「知识检索」节点内部再由 Agent 决定是否改写查询、调用哪些检索工具。这样可在局部保留灵活性，但是否适合生产仍需看评测和风险。

一个实用的工程原则是：**能用确定性 Workflow 解决的问题，就不必把决策交给 Agent。** 这是一条风险与可维护性权衡，不是对所有场景的硬性禁令。
...
👔面试官：对了，这才是关键。模型每次只提出下一步的决策和 Action/结束信号；你的代码负责检测并校验输出、执行工具、把 Observation 填回状态，再次调用模型，这才叫 ReAct 的真正实现方式。
...
最基础的是直接输出答案；CoT 是逐步处理问题的一类方法，工程接口更适合要求简洁的计划、依据或中间结果，而不是完整隐藏推理；ReAct 则把模型决策、工具行动和观察结果串成循环。

ReAct 的优势是循环结构直观、能动态利用外部工具；生产实现应观测结构化决策、工具调用和结果，不必把完整推理过程暴露给用户。
...
「推理模式」可以理解为如何组织模型的中间决策、候选路径和外部反馈，以减少多步任务的错误；不等于必须把模型的隐藏思维逐字输出。CoT、ReAct 是两种不同的组织方式，收益和风险都要按任务评测。

#### CoT是什么？

**CoT**，全称 Chain of Thought（思维链），由 Wei 等人在 2022 年提出，是最早也最简单的解法。

核心想法是让模型在输出结论前分解问题或产生中间结果。提示语可能影响这种行为，但并不保证质量，也不应作为“暴露完整推理”的承诺。更稳妥的生产接口是要求结构化计划、证据或可验证的计算结果。

如果中间结果被保留在上下文中，它会参与后续生成；但中间结果可能包含错误，模型也可能跳过步骤，所以需要规则、工具或评估器校验。

当模型输出“第一步/中间结论”并被保留到上下文时，下一步可以引用它；但这并不表示中间结论天然正确。需要把可验证计算交给工具、把证据绑定到来源，并把“理由字段”与最终结论分开记录。

CoT 有两种触发方式，理解了区别才能在实际项目中选对。

- 第一种叫 Zero-shot CoT：用提示要求模型分解问题或给出简洁步骤。它不需要示例，但格式、深度和可靠性不稳定，还可能增加输出与泄露风险。
- 第二种叫 Few-shot CoT：提供若干“输入 -> 结构化步骤/结论”的示例，让模型模仿格式。示例可以展示可公开的解题步骤或验收字段，不必包含完整隐藏推理；示例质量和 token 占用仍需评估。
...
纯 Act-only 可以让模型直接输出工具调用，不输出理由；它可能减少输出，但仍需要状态、校验和观察反馈来决定下一步，遇到复杂任务时可能更难诊断。是否需要可公开的理由字段，应按审计与安全要求设计，而不是依赖隐藏推理。

在多跳问答基准上，Act-only、CoT 和 ReAct 的相对表现会随数据集、提示、工具和评测协议变化。直觉上的差异是：只行动的系统缺少显式的计划/状态检查，遇到无关结果时可能更难纠偏；但应以同条件实验而不是固定排行榜结论判断。
...
Final Answer: （示意）产品 A 指标更高，差值为 30；真实系统还应附来源、时间和口径
```
...
许多现代模型或其适配层支持 **Function Calling / Tool Use**，可以输出结构化的工具调用；但支持的 schema、并行调用、严格模式和错误行为随模型与 SDK 变化。没有结构化工具调用时也能用文本协议，只是必须由运行时做更严格的解析、校验和拒绝。
...
规划阶段可以用在任务评测中更稳的模型，执行阶段则可尝试更快、更便宜或更专门化的模型；是否省钱取决于调用次数、输入长度、重试率和供应商价格，必须用真实流量或代表性回放测量，不能承诺固定比例。
...
面试官最想听到的核心点是两个：第一，ReAct 的本质是「决策 -> 行动 -> 观察」的循环，能动态调用外部工具；第二，这个循环是由代码框架驱动的，模型提出结构化 Action/结束信号，代码负责 schema、权限和预算校验、执行工具、把 Observation 填回状态，再把必要历史传给模型进入下一轮。可以把 Thought 作为论文术语提到，但不要承诺暴露完整隐藏推理。
...
这里有一个实际项目中常用、但需要评测验证的技巧：既然规划和执行是两个独立模块，就可以尝试使用不同的模型。规划阶段可选择在复杂拆解任务上更稳的模型，执行阶段则可选择更快或更便宜的模型，前提是结构化输出、工具参数和失败恢复仍达标。

但执行阶段是否适合小模型，要看工具 schema、任务复杂度、上下文长度和容错要求。规划只调用一次并不等于一定省钱；执行重试、长输入和失败补偿都可能抵消收益。应按总调用次数、输入/输出 token、工具成功率和端到端完成率比较，而不是预先承诺固定降幅。
...
Reflexion 的效果有多强，取决于任务是否有可靠的外部反馈（例如测试、规则或人工验收）、记忆是否准确以及反思是否真的改变了后续动作。基准数据不能直接代表业务收益；工程上应比较相同数据、模型和预算下的通过率、错误类型、额外调用与延迟。
...
下面的数字只是帮助理解输入增长的示意，不是模型或框架的性能承诺。假设有一个需要 5 步工具调用的任务，每步产生的推理和工具结果平均占 2000 token。
...
规划阶段、执行阶段和汇总阶段携带的上下文可以不同；如果执行只带当前步骤和可靠摘要，输入量可能低于把完整历史反复带上的方式，但摘要本身也有额外调用和信息损失。实际成本应按模型单价、输入/输出 token、工具耗时、重试率和失败补偿统一测量，而不是从一个示例直接推导降幅。

加了 Reflection 之后，每个需要反思的节点至少多一次评估或修订动作；若反复不通过，还会增加更多调用和工具执行。因此 Reflection 不是越多越好，应由验收指标、最大轮次、总预算和“无进展”检测共同控制，具体轮次通过评测选择。
...
拆完之后步骤之间可能有依赖关系；独立步骤可以尝试并行，但端到端收益取决于关键路径、工具延迟、并发额度、限流、失败重试和汇总开销，应通过 P50/P95、吞吐和错误率测量。
...
![](../images/1b4799cc4aa298fb846ef9aa.png)
...
![](../images/5fe6fecaf2aeffe7da9451fd.png)
...
![](../images/5bedcd322adb3c9106ccdce9.png)
...
![](../images/52973d43c5811dfd14ee9d5f.png)
...
![](../images/d403f6b8072bed0aa4cb600f.png)

如果这四步全部串行，总时间是四步之和。识别出依赖关系并行执行后，关键路径可能缩短；例如步骤 1 和 2 独立时可以并行，再等待步骤 3 和 4。实际收益取决于依赖图、工具 I/O、调度开销、限流和失败重试，应用关键路径、p95/p99 延迟和资源利用率测量，不能预设固定降幅。

要让并行真正落地，前提是先把依赖关系画成一张有向无环图（DAG），每个节点是一个步骤，边表示「依赖」。没有依赖的节点就能同时跑，依赖它们的节点要等父节点完成。这一步做得对不对直接决定了并行优化的天花板。

![](../images/fb59a05f7f2f1b4bc8ed356e.png)
...
![](../images/31c05373d78b012445aa1747.png)
...
![](../images/f7b39750428da65fae7ef3d7.png)
...
第三层是「拆完还要做的事」：分析步骤依赖关系，把独立步骤并发跑，并用关键路径、资源占用和失败恢复评估是否真的获得收益，不能预设固定降幅。
...
还有一种做法是把**不常用但重要的信息「卸载」到长期存储里**。执行过程中产生的中间结果，如果当前步骤不需要但后面可能用到，就把原文或可寻址的工件写入合适的存储，并在 state 中保留 ID、版本、来源和权限；后面再按需读取或检索，而不是默认把所有内容压成向量。这样既减轻上下文压力，也保留了可审计和精确恢复的路径。
...
短期状态可以包含 context window 里的对话历史，也可以包含外部 checkpoint、待办、预算和工具结果引用；任务结束后是清理、归档还是保留，取决于恢复和治理要求。长期数据按结构选择关系库、KV、对象/全文/向量索引等，需要语义召回时再用 embedding 检索并带回来源与权限信息。
...
![](../images/3055304241ab490225fe128e.png)

#### 短期记忆

先说**短期状态**，它通常包含每次调 LLM 时传入的 messages，以及 checkpoint 中的目标、待办、工具结果引用、预算和审批状态。你可以把它想象成 LLM 当前的「工作台」；模型可见的是应用传入的上下文和结构化决策/工具结果，不应假设完整隐藏推理一定存在或适合持久化。
...
![](../images/7168897a9fdea7cc86d4dd74.png)

#### 长期记忆

**长期记忆**常用的工具组合是结构化存储、全文/向量索引和 Embedding（向量化）；向量数据库只是需要语义召回时的一种实现，对初学者来说先理解“数据类型决定查询方式”更重要。
...
![](../images/cb3675b91fa81bf618c83407.png)
...
![](../images/946157e80ffda3fa68c4f9e5.png)
...
![](../images/5890ce0fc116afa541926e1a.png)
...
第一个雷是把长期记忆说成「存数据库靠关键词搜索」或反过来认为“必须用向量库”。更准确的回答是：结构化偏好和状态通常精确查询，文档或语义相关的非结构化内容可以用 Embedding + 向量检索，全文、过滤、时间和权限条件往往需要与它组合，并且要保留原文和证据。
...
![](../images/087f076ba671a241ad94dff3.png)
...
![](../images/5b622090da264d376c7447d6.png)
...
![](../images/578a328c9e1bc16dbbb661ec.png)
...
![](../images/9295b50d64047a893bc38b64.png)
...
![](../images/5d51f92af9f36799ebc0129d.png)
...
另外值得关注的是 **A2A**（Agent-to-Agent）一类协议。它要解决的问题是：不同团队或框架开发的 Agent 如何发现能力、传递任务、回报进度和交换结果。协议互操作不等于业务语义、身份授权、幂等、回调可靠性和数据隔离已经解决；具体治理、版本、实现数量与生产成熟度应按目标生态核对。工程上先定义清晰的任务契约和失败协议，再决定是否值得引入跨 Agent 协议。
...
这个 messages 列表受目标模型和 API 的上下文上限约束，具体容量、计费和超限行为随模型、套餐和 SDK 变化，不能把某个供应商或时间点的数字当成通用规则。超过可用预算后，应用可能截断、摘要、检索外置内容或直接拒绝请求；截断顺序也应由业务显式配置，否则早期的约束、审批和证据可能被误删。
...
除了上面这些「信息层」的压缩策略，还有一个工程上值得了解的技术叫 Prompt Caching。不同供应商和模型对缓存前缀、有效期、命中条件、隐私隔离、计费和 API 形式的支持不同，落地前应核对目标服务。
...
Prompt Caching 的思路是：如果 prompt 的一部分在多次请求之间稳定且满足服务商的命中条件，就复用这部分的计算结果，减少重复处理。它可能降低输入处理延迟或成本，但命中率、缓存粒度和收益需要通过服务商指标与实际流量验证，也要评估敏感数据是否允许缓存。

是否回本取决于命中次数、写入/读取价格、缓存有效期、前缀是否稳定以及请求的输入输出比例；不能用固定折扣或“复用两次就回本”的规则推断。Agent 还应避免把用户私密内容或跨租户内容误放进可复用前缀，并记录命中率、延迟和成本变化。
...
![](../images/354eaf96660703d13913e1ec.png)
...
![](../images/7c447174a92018ff005cdde6.png)
...
![](../images/623951ec32f71ba9c2aab4eb.png)
...
![](../images/53fff88dda45ea2f30dd44f4.png)
...
这个观点也符合一种通用的工程实践：不要一开始就堆满抽象，先用最小可观测闭环验证核心逻辑，再按状态、重试、人工介入和运维需求引入框架。框架能减少胶水代码，但抽象层也会增加排错路径，因此要保留底层日志和可复现测试。

有一个类比能很好地概括这个区别：框架是「租房」，装修好直接住，方便，但结构改不了，房东随时可能调整政策；手搓是「自建」，建起来慢，但所有结构都熟悉，改什么都能改，住着踏实。框架给你省了搭建时间，但你对这个「房子」的控制权始终有限。

![](../images/a3ef21f06ff35185453da9b4.png)
...
for i in range(max_turns):  # 手动控制最大轮次
    ...
    response = client.chat.completions.create(
        model="provider:model-id", messages=messages, tools=tool_schemas
    )
...
![](../images/ec427edc30c3cbb9c89c2c45.png)
...
![](../images/6dd125befba88277f0a6665d.png)
...
![](../images/dbe073b41d33f30ca9f043f2.png)
...
![](../images/1ef0c0b262fb38610cb032bc.png)
...
- CoT 是让模型按步骤处理问题，可输出简洁的计划、依据或中间结果，但不等于必须公开完整隐藏推理；
- ToT 是让它同时探索多条推理路径，选最优的继续深入；
- GoT 是图结构推理，推理节点可以复用和合并，适合更复杂的任务。

工程上是否采用逐步推理要由任务评测决定；ToT/GoT 需要更多候选、评估和状态管理，调用与成本随搜索配置增长，只有在分支探索或结论汇聚带来可测收益时才值得引入。

### 📝 详细解析

要理解为什么需要规划能力，先看 LLM 在没有任何规划机制时是怎么运作的。

普通的问答调用通常直接返回面向用户的答案，应用不一定能看到模型内部的中间推理。对需要多步推导的任务，可以让模型输出可验证的计划/中间结果，或把计算、检索交给工具，再用验收器检查。
...
「规划能力」要解决的是目标、依赖、验收条件和资源约束的组织问题；可以通过显式计划、状态机、工具反馈和重规划实现，而不等同于把隐式推理逐字显式化。
...
CoT 的全称是 Chain of Thought（思维链），核心思路是让模型按多个中间步骤处理问题。提示语可能影响输出，但不保证可靠，也不应承诺输出完整隐藏推理；生产接口更适合要求结构化计划、依据和可验证结果。

为什么这么简单的改变就有效？

如果中间结果被应用保留到上下文，它会影响后续生成；但中间结果可能错误，模型也可能跳步，因此仍需要工具、规则、证据和评估器校验，不能把“写出来”当作正确性的保证。
...
- 第二种叫 Few-shot CoT，给几个“输入 -> 结构化步骤/结论”的例子，让模型模仿格式；示例可以展示可公开的解题步骤或验收字段，不必包含完整隐藏推理，效果和成本仍需在目标任务上验证。
...
这个「生成 -> 评估 -> 剪枝」的循环，让 LLM 可以探索多条路并在评估不通过时回退。代价是调用次数大致随候选数、搜索深度和评估次数增长，还可能伴随更长的上下文和工具执行；应按搜索配置、模型价格、延迟预算和任务收益测量，而不是使用固定倍数。
...
GoT 能表达比树更丰富的依赖、汇聚和复用，但落地复杂度也更高：需要定义节点状态、合并规则、失败恢复和观测。是否适合生产不能只按“学术/生产”二分，应看任务是否确实需要分支合并，以及实现是否能通过评测和运维要求。
...
工程上怎么选？逐步推理提示不等于可靠规划，也会增加输出长度或暴露不必要的推理细节；应让模型输出可验证的计划、结论和依据，而不是依赖展示完整思维过程。ToT 只有在分支探索能带来可测收益时才值得引入；GoT 只有在中间结论确实需要汇聚/复用时才有价值。两者都要配置候选上限、评估器、超时、预算和回退路径。
...
工程上 Plan-and-Execute 的一个好处是规划和执行可分别评测、限额和选模型：规划阶段可使用在复杂拆解上更稳的模型，执行阶段可尝试更快或更便宜的模型，但必须验证工具参数、失败恢复和总体完成率。具体框架是否提供现成模板、持久化和人工介入能力，应按目标版本文档核对。
...
最容易被忽略的考点是工程取舍：分支搜索和图式推理会带来额外调用、状态和评估复杂度，收益必须由任务级评测证明；计划执行则要处理计划过时、部分失败和重新规划。回答时说清楚候选上限、预算、验收标准和回退策略，比背固定倍数更可靠。
...
![](../images/ba4317b129784601d54754c4.png)
...
![](../images/5a9d7327e03ecc6beae53c87.png)
...
![](../images/64b3a8bae4ece292570965d2.png)
...
![](../images/86beeca7cd0fdc8926a456fd.png)
...
第二个是 LATS（Language Agent Tree Search）这类把反思与树搜索结合的研究方法。它可以用搜索策略探索多条路径，并把执行后的评估/反思反馈给后续探索；代价是状态、评估、工具执行和预算管理都更复杂。是否适合业务要看任务是否有可靠的评分器和可接受的搜索预算，不能仅凭论文范式判断生产成熟度。
...
什么场景不值得开？简单直接且已有确定性校验的任务，反思可能只是重复开销。实时性要求高的场景也要谨慎，因为一次反思通常至少增加一次评估或修订动作，延迟是否可接受应由目标 SLO 和线上测量决定。

最重要的是防死循环，必须设置硬性的最大轮次、总预算和“无进展”检测，不能依赖 LLM 自己判断停止。LLM 可能陷入“为了改而改”的循环，因此评估器还应能返回 PASS、FAIL 或 NEEDS_HUMAN，并记录每轮差异以便审计和回退。

![](../images/fdc45b71684b2faa5d9c01a7.png)

最后要对整体代价有清醒认知：每轮反思可能包含评估、修订以及重新执行工具，调用数、延迟和成本都会增加；具体增加多少取决于是否通过、是否重试和上下文长度。反思是需要用任务级评测证明收益的增强机制，不应默认每一步都启用。
...
第四，最容易被遗漏的工程要点是防死循环：必须硬性设置最大轮次、预算、超时和人工升级条件，不能依赖 LLM 自己停止。
...
![](../images/9ffa68f380f201bae0619c40.png)
...
![](../images/7020438eb0864dbd78928450.png)
...
![](../images/18c57b461fb0133ef279bb71.png)
...
![](../images/5a398da931aa7ed53c0c5cb5.png)

两种路由策略的对比可以用一张图来理解：

![](../images/d8cf5e726718557dc2086a20.png)
...
![](../images/798ea19b636df14122228845.png)
...
![](../images/57c59b6d70ea35165f7814cd.png)
