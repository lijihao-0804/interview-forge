...
> 原文：[Claude Code 使用教程：新手入门必学的基础技巧](https://xiaolinnote.com/claudecode/basics/cc_use.html) · 小林面试笔记
...
但要说哪款工具，确实可能改变终端编程习惯，我会把 **Claude Code** 作为一个值得评估的例子；效率收益取决于任务、权限、模型、代码库和验证流程，不能预设翻倍。
...
是否考察取决于岗位和面试官；后端或 Agent 岗位可能询问实际使用、工具调用、权限和可靠性，但不应把固定概率当作普遍规律。
...
Claude Code 可能提供多种权限/工作模式；名称、数量、快捷键和默认行为会随版本变化，使用前先查看当前帮助和设置。

![](../images/dfa1bb12dcd8a3472a5a2415.png)

按 **Shift+Tab** 可以在三个模式之间切换：

- **Normal/确认类模式**：对未授权的工具或副作用请求进行确认；不是每一步都必然弹窗，已有授权、规则和工具类型会影响行为

![](../images/c3df141ca999468e218b72c5.png)

- **Auto-accept 类模式**：可能减少文件编辑等低风险操作的确认，但命令、网络、删除和外部写入仍应按权限与版本核对

![](../images/d434583e9b4a16179a117a5a.png)

- **Plan 类模式**：通常用于只读分析和计划；是否能执行命令、写入文件或触发外部副作用取决于当前配置，不能只凭名称假定

![](../images/c959f13eca14bca86f0ff215.png)

**新手建议**：先用最保守的确认模式，检查工具名、参数和目标；熟悉后也应在可恢复分支、最小权限和 CI 校验下逐步减少确认。
...
这种「目标、约束、验收标准清晰」的提示词通常更易执行，但提示词越长不一定越好；应给出必要上下文、边界和验证方式。
...
- 第一个：按当前权限策略执行修改，可能减少确认
- 第二个：保留人工审核修改或工具调用
- 第三个：给 Claude 反馈，调整它的修改方案（比如改哪里、怎么改）

某些版本会把计划写入 Markdown 或其他任务状态中，帮助恢复进度；这不是事务状态机，计划可能过期，执行前仍需重新检查依赖、权限和验收条件。
...
其中可能有名为 **frontend-design** 的 Skill 或插件示例；名称、来源、许可和安装方式会变化，第三方 Skill 应先审查内容、脚本和网络权限。
...
执行后，如果当前版本配置了相应插件源，客户端可能下载并安装该 Skill；不要把示例命令视为所有环境都可用。
...
如果 Skill 被正确发现并加载，模型会依据其中的指令尝试重构页面；它不保证设计质量，也不应绕过测试、可访问性和人工审查。

![](../images/b78c67ef8260fb88e30e839e.png)

它可能调整配色、布局、字体和交互动画；最终质量取决于 Skill、现有代码、设计要求和验收，不应承诺“完全不一样”或自动达到专业水平。
...
这样是在提示中给出明确入口，但不等于绝对禁止访问或修改其他文件；需要配合权限规则、路径校验和 diff 审查。
...
但如果你用 @ 指定文件，通常能减少搜索范围；模型仍可能读取导入、测试、配置或其他依赖以完成任务。

例如只指定一个组件文件可能比扫描整个仓库更省上下文，但不代表只读取固定行数；上下文成本还取决于工具结果、模型版本和任务。

所以可以养成习惯：**在目标明确时用 @ 缩小范围**，再允许模型按需要补读依赖；不要用固定的上下文百分比作为所有版本的触发阈值。
...
为什么会这样？复杂任务需要更明确的状态、边界和验收，也可能需要更高的推理预算；具体 effort 档位和命令由模型/版本提供，不能把某个关键词定义成普遍的“最高挡”。
...
在支持该关键词的版本中，`ultrathink` 可能提高这一轮的思考努力等级；是否生效、对应哪一档和是否恢复原设置，应以当前版本文档和实际 `/effort` 状态为准。

![](../images/4f969b1b44fe8284cdab3ffc.png)

执行的时候，你也能看到，它会「先梳理方案再动手」，而不是边做边看。

旧教程中的 `think`、`think hard`、`think harder`、`ultrathink` 语义可能与当前版本不同；不要据此推断当前行为，直接查看当前 `/effort` 与模型支持的设置。

如果希望调整整体推理预算，应使用当前版本支持的 `/effort` 或模型设置，并在目标任务上比较质量、延迟和 token 成本；临时关键词是否只影响一轮要以实际版本为准。
...
有没有办法让 Claude 在后续会话中持续获得项目背景？

可以用 **`CLAUDE.md`**，但它是指导文本，不是永久记忆或强制安全边界。

### CLAUDE.md 是什么？

`CLAUDE.md` 常放在项目根目录，Claude Code 会按当前版本的目录发现、工作目录和设置读取项目指令；加载位置、继承关系和注入时机应以当前文档为准。

![](../images/c1ccaef0635f676a28ecf88e.png)

你可以把它理解为项目级上下文入口；模型仍需要读取文件、验证命令和检查规则是否过期，不能假定它“知道”全部项目状态。
...
Claude 可能扫描项目结构并生成一份 `CLAUDE.md` 草稿；生成内容需要人工审查，避免把密钥、过时信息或错误命令写入项目规则。
...
如果生成内容语言或格式不符合要求，可以让 Claude 改写或直接人工编辑 `CLAUDE.md`，修改后仍应检查事实、路径和命令。

![](../images/46cbe4c999034648d27c3460.png)

生成的 `CLAUDE.md` 可以记录番茄钟应用的命令、架构和约定；后续是否自动加载以及加载多少由产品规则决定，常驻信息仍要保持精简。

![](../images/c4a9c379dc1b84af85f6fc50.png)

`CLAUDE.md` 本质上是一个 Markdown 文档，可以人工修改；对权限、危险命令和发布流程，仍应使用程序化策略、审批和 CI，而不是只写文字。

### 注意事项

不过，`CLAUDE.md` 不是越长越好，常驻内容会占用上下文预算，原则是**稳定、必要、可验证**。

![](../images/40ba374b286618661a277e70.png)

一份好的 `CLAUDE.md` 可以包含这几部分：
...
要注意的是，`CLAUDE.md` 有一个很重要的原则：**每条信息都要问自己「如果删掉这条，会不会让 Claude 犯错？」如果不会，就不常驻；如果涉及安全边界，还要落到程序化控制。**

![](../images/17bc97dbe559cdac3ecf465e.png)

很多人喜欢在 `CLAUDE.md` 里堆一大堆东西，结果文件太长会增加输入成本并稀释重点。

比如下面这些内容通常不应该写进 `CLAUDE.md`：

- 详细的 API 文档（太长，占上下文，应该用 @ 引用具体文件）
- 每次都变的临时需求（用 `/compact` 或 `/clear` 就行）
- 过于泛泛的描述（比如「写好代码」「好的代码」这种 Claude 本来就知道的事）

`CLAUDE.md` 是一个**活文档**。随着项目开发，应更新命令、技术栈、约定和已知坑，并删除过期内容。

有了这份文件，`/clear` 后通常可以减少重复解释；重新启动时是否加载仍取决于当前目录与版本规则。

**一句话总结：`CLAUDE.md` 是项目指令和背景的持久化入口，不是永久记忆；保持精简、准确，并把强制约束放到权限、Hook、CI 或外部策略。**
