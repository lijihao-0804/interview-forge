...
<ol class="lang-steps"><li>pA、pB 分别从 A、B 头出发，走完自己的就切到对方的链</li><li>两者走过总长度相同（a+c+b 与 b+c+a），必然同点</li><li>在交点相遇返回；无交点会同时走到 null</li></ol>
...
func getIntersectionNode(headA, headB *ListNode) *ListNode {
    pa, pb := headA, headB
    for pa != pb {
        if pa != nil { pa = pa.Next } else { pa = headB }
        if pb != nil { pb = pb.Next } else { pb = headA }
    }
    return pa
}
```
...
struct ListNode* getIntersectionNode(struct ListNode* headA, struct ListNode* headB) {
    ...
    while (pa != pb) {
        pa = pa ? pa->next : headB;   /* 走完自己的就换对方的 */
        pb = pb ? pb->next : headA;
    }
    return pa;
}
