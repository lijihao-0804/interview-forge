...
> **版本与事实边界**：本章保留了入门教学中的历史截图、模型名和控制台操作。模型名称、上下文长度、价格、额度、网页布局和 API 参数都会变化；这些内容只能作为当时的示例，运行或面试回答前要以提供商官方文档为准。API Key 只放在环境变量或 Secret Manager 中，不要写入仓库、截图或日志。
...
“模型相当于几岁小孩/成年人”的说法缺少统一、可复现的测量标准，不能作为技术结论。更稳妥的表述是：不同模型在不同基准、语言和任务上的能力差异很大，强项可能包括文本生成、代码或工具调用，但仍会犯事实、推理、鲁棒性和安全错误。
...
近几十年来，神经网络架构和训练方法不断演进。早期模型通常受限于架构、显存和训练数据，能处理的上下文较短；上下文长度并不是某种架构固定不变的属性，而取决于模型设计、训练方式、推理实现和服务商限制。

Transformer 在训练阶段能较好地利用并行计算，并通过注意力机制建模序列关系；但上下文窗口、长上下文效果和成本随模型与服务而异，窗口更长也不等于模型一定能正确利用所有信息。
...
到了现代，互联网和数字化系统提供了大量文本、代码、图像与交互数据。但训练集通常经过筛选、去重、授权/合规处理和质量控制，不能笼统地说模型“学习了整个互联网”，也不能把训练数据规模直接等同于事实可靠性。
...
> 下表是历史示例，不是当前产品目录。模型名、产品入口和可用区域会变化；选型时应查看对应提供商的最新模型卡、服务条款和 API 文档。
...
这些平台的收费、免费额度、计费单位和有效期各不相同，并且会调整，应以官方价格页和账户页面为准。Token 是模型处理文本的离散单位，中文字符与 token 并非固定一对一，不能用“一个汉字通常等于 2 个 token”做精确预算。
...
DeepSeek API 是否收费、是否需要预充值以及可用额度会随账户、地区和官方政策变化；注册和调用前应以当前官方控制台与价格说明为准。
...
访问公共大模型常通过 HTTP API，但接口、认证、工具调用、流式事件、错误码和限流语义并不统一。许多服务提供“OpenAI-compatible”入口，只代表一组兼容约定，不代表所有参数和行为完全一致。
...
这里的 7B、32B、671B 等通常表示参数规模，但参数更多不保证在每项任务上都更强，还要看训练数据、架构、量化和评测。Ollama 中可用的模型标签、量化方式和本地硬件要求会变化；量化模型与蒸馏模型是不同概念，不能只按官网模型名直接比较。
...
很多大模型服务提供基于 HTTP/JSON 的 OpenAI-compatible 接口，因此请求路径和常见字段可能相似；但兼容范围、工具调用、流式格式、错误处理和鉴权细节仍可能不同。具体行为必须查看目标服务的官方 API 文档。
...
OpenAI 的接口形式对生态影响很大，许多服务提供 OpenAI-compatible 入口。但这是一种生态约定，不是所有大模型 API 都完全兼容；使用 SDK 的 `base_url` 还要核对目标服务支持的端点、参数、鉴权、流式响应和错误语义。
