...
# ROOT：学习站根目录，由共享路径模块固定，不随脚本目录移动而变化。
...
34|在排序数组中查找元素的第一个和最后一个位置|二分查找|左右边界二分|O(log n)|O(1)|分别寻找第一个大于等于 target 和第一个大于 target 的位置
...
EASY = {1, 20, 21, 35, 70, 94, 101, 104, 108, 118, 121, 136, 141, 160, 169, 206, 226, 234, 283}
...
def build() -> None:
    ...
    render_dashboard()
    library_builder = ROOT / "scripts" / "build" / "build_library.py"
    ...
    html_builder = ROOT / "scripts" / "build" / "build_html_site.py"
