...
#results.chapter-grid{display:block}
.search-group{display:grid;grid-template-columns:repeat(auto-fill,minmax(min(270px,100%),1fr));gap:13px;margin-bottom:22px}
.search-group-title{grid-column:1/-1}
...
:root{--diagram-surface:#fbfcff;--diagram-node:#f1f2ff;--diagram-node-border:#7775dc;--diagram-decision:#fff7e8;--diagram-decision-border:#dfa34c;--diagram-cluster:#f7f9fd;--diagram-cluster-border:#d4dbe7;--diagram-line:#8490a3;--diagram-label-bg:#fbfcff;--diagram-node-text:#344056;--diagram-glow:#f1f2ff}
@media(prefers-color-scheme:dark){:root{--diagram-surface:#1b2230;--diagram-glow:#262c4a;--diagram-node:#263042;--diagram-node-border:#8b88f0;--diagram-decision:#3a2f1e;--diagram-decision-border:#dfa34c;--diagram-cluster:#1f2735;--diagram-cluster-border:#3d4a5e;--diagram-line:#5d6b80;--diagram-label-bg:#1b2230;--diagram-node-text:#dce4f2}}}
.mermaid-diagram{background:radial-gradient(circle at 18% 8%,var(--diagram-glow) 0,transparent 43%),var(--diagram-surface)}
...
/* ===== 表格统一样式（横线为主 + hover 高亮，后写覆盖旧的 per-cell 边框） ===== */
.reader table,.chapter-list table{width:100%;max-width:100%;border-collapse:separate;border-spacing:0;
font-size:.93em;border:1px solid var(--line);border-radius:var(--radius-2);overflow:hidden}
.reader th,.chapter-list th{background:var(--soft);font-weight:700;color:var(--text)}
.reader th,.reader td,.chapter-list th,.chapter-list td{padding:9px 13px;border:0;
border-bottom:1px solid var(--line);text-align:left;vertical-align:top}
.reader tbody tr:last-child td,.chapter-list tbody tr:last-child td{border-bottom:0}
.reader tbody tr{transition:background .12s ease}
...
def render_markdown(text: str) -> str:
    ...
    rendered = markdown.markdown(
        prepared,
        extensions=["extra", "sane_lists", "fenced_code", "tables", "toc"],
    )
...
def build() -> None:
    ...
    for asset_name in ("ai-launcher.css", "ai-page-context.js", "ai-launcher.js"):
        shutil.copy2(HOT100_ROOT / "assets" / asset_name, OUTPUT_ROOT / "assets" / asset_name)
    ...
    # 更好定位。
    mermaid_source = HOT100_ROOT / "tools" / "vendor" / "mermaid-11.16.1.min.js"
    ...
    for definition in LIBRARY_MODULES:
        ...
        # 章节页展示的“最后更新”：取源笔记文件的修改时间（内容真实变更时才前进）
        book_updated = datetime.fromtimestamp(source.stat().st_mtime).strftime("%Y-%m-%d")
...
else{{var ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();var ok=document.execCommand('copy');ta.remove();done(ok)}}
}});
pre.appendChild(btn);
}});
