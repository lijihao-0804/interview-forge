...
项目内置**学习记录、间隔重复复习、学习轨迹、限时模拟、力扣提交同步、学习中控台、多用户与学习社区**等能力，形成“学 → 练 → 复盘 → 复习”的完整闭环；所有数据只保存在本机 SQLite，完全离线可用。
...
- **题解多语言切换**：题页顶部语言条可在 Java / C++ / Python / Go / C 间切换，所选实现**原位替换 Java 版的位置**（紧跟推导、在复杂度之前），语言偏好随账号保存；补充语言覆盖灵茶山艾府题解（MIT License）与站内标准实现；
...
- 18 个交互可视化演示内嵌到对应题解/章节（哈希表、双指针、链表、锁升级、TCP 握手挥手、ReadView 版本链等），只展示关键状态变化，一键播放/分步；演示区**跟随站内深色模式**实时切换。
...
- Mermaid 流程图离线渲染（品牌配色、随深浅色主题自适应），单图语法异常只标记该图并输出诊断，不阻断同页其余图；Pygments 代码高亮（Java/Python 等）、章节内嵌交互演示；
...
- **60 天复习边界**：逾期超过 60 天的题不再塞进“今日待复习”，转入“需重学”（进今日计划池），题卡显示紫色「需重学」徽标，避免长期欠账刷屏；
- 面板“今日待复习”只显示 Hot 100 题目；书架章节在书架各模块页展示，互不混排（可开启设置 `review_include_contents` 在面板追加独立折叠子区）。

### 5. 今日计划

- 组合“已排期 + 未学习 + 需重学 + 轮数较少”并标注理由；点“换一组”随机抽取新题；
- 题卡可**“纳入明天计划”**：把任意题固定到明天的计划首位（已排期优先展示）；
...
- **AC 自动推进复习轮次**（AC 即记一轮并推进下次复习）；**WA 自动标记薄弱并提前进入“今日待复习”**。

#### 服务器部署场景（Cloudflare 风控）

- 力扣官网对海外机房 IP 有 Cloudflare"人机验证"拦截：标准库请求一律 403，**与会话是否有效无关**；
- 项目已内置应对：检测到服务器装有 `curl_cffi` 时自动改用 Chrome TLS 指纹请求（`pip3 install curl-cffi` 即可），未安装则回退标准库；
- 连接测试会把“出口 IP 被风控”与“会话过期”分开提示，不再误判为 Cookie 失效；
- 力扣状态查询结果按会话缓存 10 分钟，避免频繁探测触发风控。
...
- 面板数据经 `/api/bootstrap` 一次拉齐（面板 + 今日计划 + 设置），力扣状态带 10 分钟缓存，弱网/远程访问也流畅；
- 页面左右边距统一 200px（1400px 以下自适应回退，手机端另有专属边距）；
- 本地服务监听 `0.0.0.0:8765`，局域网内其他设备也可访问（本机地址 http://127.0.0.1:8765/）。

### 11. 学习中控台（cockpit.html）

- 打开首页即见的"驾驶舱"：实时时钟 + 按时段问候 + 极光渐变背景；
- Bento 概览卡：今日待复习、今日计划（各最多展示 3 条，超出滚动查看）+ 快速入口 + 一句话简介；
- 与学习面板、书架、力扣连接互通，作为每天开始学习的落地方。

### 12. 多用户与学习社区

- **注册码注册 + 登录**：密码 scrypt 加密存储，会话 Cookie（HttpOnly / SameSite=Lax），支持修改自己密码；
- **个人资料**：昵称、头像、题解语言偏好（随账号漫游）；
- **学习聊天室**与**问题反馈**：站内交流与缺陷上报，反馈有 open/resolved 状态与处理备注；
- **管理后台**（仅管理员）：用户清单、注册码签发、反馈处理，"最近活跃"显示用户最近一次请求时间（而非仅登录时间）。

### 13. 深色模式

- 三态切换：跟随系统 → 浅色 → 深色（左下角悬浮按钮，localStorage 记忆）；
- 全站覆盖：阅读页、书架章节、面板、中控台、登录注册、Mermaid 图、代码高亮与**内嵌交互演示**（iframe 内实时跟随切换）。
...
│  ├─ extract_lang_solutions.py      多语言题解提取管线（一次性工具）
│  ├─ fix_lang_solutions.py          多语言题解数据修复（一次性工具）
