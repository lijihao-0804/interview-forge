...
正式使用请访问 [https://hot100.xyz/](https://hot100.xyz/)；不要把本地启动脚本当作线上入口。本地开发或线上故障应急维护时，才使用根目录的 `启动学习站.cmd` 启动本地服务，命令窗口需要保持开启；结束后按 `Ctrl+C` 停止。

线上学习记录按账号保存在服务器端 SQLite 数据库；本地开发生成的 `data/hot100-study.db` 仅用于本地调试和备份演练，全量重建题解不会覆盖它。
...
6. 在开发环境启动服务后进行本地验证；发布后再通过 `https://hot100.xyz/` 确认新增入口、题名、专题、轮次记录和可视化链接正确。
...
- [ ] 所有困难题都有与实际方法一致的内嵌演示。
- [ ] 学习面板能搜索到新题，专题筛选正确。
- [ ] 开发环境服务启动后健康检查正常；线上发布后登录 `https://hot100.xyz/`，打开题解会增加当天看题记录。
- [ ] 同一题同日多次 AC 只计一轮，隔天再次 AC 轮次才增加。
