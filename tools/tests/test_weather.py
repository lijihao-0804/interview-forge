...
class WeatherTests(unittest.TestCase):
    ...
    def test_ui_contract(self):
        ...
        self.assertIn("border-radius:20px", html)
        self.assertIn("weather-breathe 16s", html)
        self.assertIn('id="clock-main"', html)
        self.assertIn('id="clock-seconds"', html)
        self.assertIn("font-variant-numeric:tabular-nums", html)
        self.assertIn('class="learning-pill">识别模式', html)
        self.assertIn('class="weather-datum"><span>最高</span>', html)
        self.assertIn('class="weather-datum"><span>最低</span>', html)
        self.assertIn('id="weather-refresh" title="刷新天气" aria-label="刷新天气">↻', html)
        self.assertIn('document.getElementById("weather-reset").hidden = data.location.mode === "default"', html)
        self.assertIn("formatWeatherUpdate", html)
        self.assertIn('refresh.classList.toggle("is-loading", loading)', html)
        hero_css = html[html.index("/* ===== Hero"):html.index("/* ===== Bento")]
        self.assertNotIn("100vh", hero_css)
