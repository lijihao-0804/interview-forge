...
.shell{width:calc(100% - 100px);margin:auto;padding:30px 0 56px}
...
<main class="shell">
  ...
  <section class="review-section" aria-labelledby="pickTitle">
    <div class="review-head"><h2 id="pickTitle">今日计划</h2><button id="pickAgain" class="round-button" data-online-action type="button">换一组</button></div>
  ...
  <section class="review-section" aria-labelledby="weakTitle">
    <div class="review-head"><h2 id="weakTitle">薄弱清单</h2><a class="review-summary" href="books/hot100/00-总览/05-错题本.html" target="_blank" rel="noopener noreferrer">错题本页 →</a>
      <div class="export-buttons">
        ...
        <button class="round-button" data-online-action type="button" data-export="records">导出记录 JSON</button>
        <button class="round-button" data-online-action type="button" data-export="db">备份数据库</button>
        <button class="round-button" data-online-action type="button" data-export="weekly">导出周报</button>
      </div>
    </div>
...
function renderTrend(){
  ...
  try{
    ...
    if(trendChart){
      trendChart.setSize({width,height:210});
...
async function exportData(kind){
  try{
    const response=await fetchWithTimeout(`/api/export?kind=${kind}`,{cache:'no-store'});
...
async function mockStart(){
  ...
  try{
    const response=await fetchWithTimeout(`/api/mock?count=${count}&category=${encodeURIComponent(category)}&difficulty=${encodeURIComponent(difficulty)}`,{cache:'no-store'});
...
window.addEventListener('resize',()=>{
  ...
  const el=document.getElementById('trend');
  const width=Math.max(240,el.clientWidth||320);
  if(Math.abs(width-trendWidth)>1){trendChart.setSize({width,height:210});trendWidth=width;}
});
...
async function refresh(){
  try{
    ...
    document.getElementById('connection').textContent='学习服务已连接';
    document.getElementById('serverNotice').hidden=true;
  }catch(error){
    state.online=false;
    document.getElementById('connection').classList.remove('online');
    document.getElementById('connection').textContent='暂时无法连接学习服务';
