...
def _weather_key_lock(key: tuple[object, ...]) -> threading.Lock:
...
def set_weather_preference(username: str, payload: dict[str, object]) -> dict[str, object]:
    ...
    with closing(connect_auth()) as connection:
        ...
        connection.execute(
            ...
            (row["id"], mode, display_name, latitude, longitude, timezone_name, now_iso()))
...
def _fetch_weather(preference: dict[str, object]) -> dict[str, object]:
    payload = _weather_http_json("https://api.open-meteo.com/v1/forecast", {
    ...
    try:
        ...
        result = {
            ...
            "updated_at": str(current.get("time") or now_iso()), "stale": False,
...
def weather_for_user(username: str) -> dict[str, object]:
    preference = get_weather_preference(username)
    ...
    try:
        ...
        if cached and now - cached[0] <= ttl:
            return _weather_response(cached[1], preference)
        try:
            result = _fetch_weather(preference)
...
def search_weather_locations(query: str) -> list[dict[str, object]]:
    ...
    lock = _weather_key_lock(("search", cache_key))
    with lock:
        with _WEATHER_CACHE_LOCK:
            cached = _WEATHER_SEARCH_CACHE.get(cache_key)
